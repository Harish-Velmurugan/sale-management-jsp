...
<body>
	...
	try {
		...
		"select Order_Id,CreatedAt,Total,Discount from Orders join Customers on Orders.Customer_ID = Customers.Customer_ID where Customers.Customer_ID=?");
		...
		while (rs.next()) {
			out.println(rs.getInt(1) + " &emsp;&emsp;&emsp;&ensp;|| " + rs.getString(2) + " || " + rs.getInt(3) + " || "
			+ rs.getInt(4));
	%>
	<br>
	<%
	...
	<form action="viewOrderDetails.jsp" method="POST">
		<input type="number" name="order_ID" /> <input type="submit"
			name="submit" />
	</form>
	<%
